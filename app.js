...
const categoriasRota = [
    { chave: 'trocas', nome: 'TROCAS' },
    { chave: 'coleta_distante', nome: 'COLETA DISTANTE' },
    { chave: 'encher_na_hora', nome: 'ENCHER NA HORA' },
    { chave: 'colocacoes', nome: 'COLOCAÇÕES' },
    { chave: 'retiradas', nome: 'RETIRADAS' },
    { chave: 'compactadora', nome: 'COMPACTADORA' },
];
...
window.carregarRotasDia = async function() {
    ...
    const selectMotorista = document.getElementById('rotaMotorista');
    if (selectMotorista) {
        const escolhido = selectMotorista.value;
        selectMotorista.innerHTML = '<option value="">Selecione...</option>';
        window.motoristas.forEach(nome => {
            selectMotorista.innerHTML += `<option value="${nome}">${nome}</option>`;
        });
        if (window.motoristas.includes(escolhido)) selectMotorista.value = escolhido;
    }
...
window.renderizarRotasDia = function(rotas) {
    ...
    window.motoristas.forEach(nome => {
        porMotorista[nome] = {};
        categoriasRota.forEach(cat => porMotorista[nome][cat.chave] = []);
    });

    rotas.forEach(item => {
        if (!porMotorista[item.motorista_nome]) {
            porMotorista[item.motorista_nome] = {};
            categoriasRota.forEach(cat => porMotorista[item.motorista_nome][cat.chave] = []);
        }

        const categoria = item.categoria || 'trocas';
        if (!porMotorista[item.motorista_nome][categoria]) {
            porMotorista[item.motorista_nome][categoria] = [];
        }

        porMotorista[item.motorista_nome][categoria].push(item);
    });

    board.innerHTML = '';

    Object.entries(porMotorista).forEach(([motorista, grupos]) => {
        const temServico = Object.values(grupos).some(lista => lista.length > 0);
        if (!temServico) return;

        const coluna = document.createElement('div');
        coluna.className = 'rota-motorista-coluna';

        coluna.innerHTML = `
            <div class="rota-motorista-header">
                <strong>${motorista}</strong>
            </div>
        `;

        categoriasRota.forEach(cat => {
            const bloco = document.createElement('div');
            bloco.className = 'rota-categoria-bloco';

            bloco.innerHTML = `
                <div class="rota-categoria-titulo">${cat.nome}</div>
                <div class="rota-categoria-lista">
                    ${(grupos[cat.chave] || []).map(window.htmlCardRota).join('')}
                </div>
            `;
    ...
    if (!board.innerHTML.trim()) {
        board.innerHTML = '<div class="text-center text-slate-400 font-bold py-10">Nenhum serviço programado para esta data.</div>';
    }

    lucide.createIcons();
};

window.htmlCardRota = function(item) {
    const classeStatus = {
        realizado: 'rota-card-verde',
        carro_quebrado: 'rota-card-vermelho',
        cancelado: 'rota-card-laranja',
    }[item.status_celula] || 'rota-card-verde';

    const classeTexto = {
        troca: 'rota-texto-preto',
        colocacao: 'rota-texto-vermelho',
        retirada: 'rota-texto-azul',
    }[item.tipo_texto] || 'rota-texto-preto';

    const qtd = item.quantidade ? `<div class="rota-qtd">${item.quantidade}</div>` : '';
    const obs = item.observacao ? `<div class="rota-obs">${item.observacao}</div>` : '';

    return `
        <div class="rota-servico-card ${classeStatus}">
            <button class="rota-delete" onclick="window.excluirServicoRota('${item.id}')">
                <i data-lucide="x" class="w-3 h-3"></i>
            </button>
            <div class="${classeTexto}">${item.descricao}</div>
            ${qtd}
            ${obs}
        </div>
    `;
...
window.salvarServicosColadosRota = async function() {
    const dataRota = document.getElementById('dataRotaDia')?.value || getHojeStr();
    const motorista = document.getElementById('rotaMotorista')?.value;
    const categoria = document.getElementById('rotaCategoria')?.value || 'trocas';
    const status = document.getElementById('rotaStatus')?.value || 'realizado';
    const tipoTexto = document.getElementById('rotaTipoTexto')?.value || 'troca';
    const quantidade = parseFloat(document.getElementById('rotaQuantidade')?.value || 0) || null;
    const texto = document.getElementById('rotaServicosColados')?.value || '';
    ...
    if (linhas.length === 0) {
        alert('Cole pelo menos um serviço.');
        return;
    }

    const registros = linhas.map((descricao, index) => ({
        data: dataRota,
        motorista_nome: motorista,
        descricao,
        quantidade,
        categoria,
        status_celula: status,
        tipo_texto: tipoTexto,
        ordem: index + 1,
    }));
    ...
    document.getElementById('rotaServicosColados').value = '';
    document.getElementById('rotaQuantidade').value = '';
...
window.excluirServicoRota = async function(id) {
    if (!confirm('Remover este serviço da rota?')) return;
