// =============================================================
        ...
        import {
        calcularDiasCorridosERestantes,
        calcularRunRate,
        calcularGpsMeta
        } from './src/business/bi.js';
        // =============================================================
        ...
        function mapearLancamentosParaBanco(lancs) {
            ...
            (lancs || []).forEach(l => {
                ...
                banco[l.data][nomeMotorista] = {
                    ...
                    pontos: (l.observacao && l.observacao.includes('[EXTRA R$ 20]')) ? 0 : l.quantidade_servicos,
        ...
        window.gerarRankingMensal = function () {
            ...
            // Colocamos o Math.ceil para sempre arredondar as caixas que faltam para CIMA
            const faltam          = Math.ceil(Math.max(0, meta - feitas));
            ...
            if (faltam > 0) {
                // Usa Math.ceil no resultado para que se faltar "1.1" mostre "2"
                const calcVisual = Math.ceil(window.motOutros.includes(mot.nome) ? faltam / 2 : faltam);
    ...
    lucide.createIcons();
        ...
        return;
            ...
            try {
                ...
                const isExtraInput = document.getElementById('isExtra')?.checked ?? false;
        ...
        const pontosExistente = lancamentoExistente.tipoVeiculo === 'poli_duplo' ? (lancamentoExistente.servicos || 0) * 0.5 : (lancamentoExistente.tipoVeiculo === 'cacamba' ? (lancamentoExistente.servicos || 0) * 2 : (lancamentoExistente.servicos || 0));
                ...
                let valorFinal = valorBase + valorExtraFinal;
    ...
    if (isExtraInput) {
        ...
        observacaoFinal = "[EXTRA R$ 20] " + observacaoFinal; // Etiqueta pro sistema lembrar
            ...
            if (el) el.value = '';
                ...
                const elExtra = document.getElementById('isExtra');
...
${window.usuarioAtualFuncao === 'operador' ? '' : `<button class="btn-delete" onclick="window.deletarLancamentoEspecifico('${dataEscaped}')">Excluir</button>`}
    ...
    if (window.usuarioAtualFuncao === 'operador') {
        ...
        window.atualizarResumosDoMotorista = function () {
            ...
            if (dataEstaNoMes(dataStr, anoMesFiltro) && dadosDia[window.motoristaSelecionado]) {
                ...
                // Verifica se tem a tag do Extra
                const isExtra = r.observacao && (r.observacao.includes('[EXTRA R$ 20]') || r.observacao.includes('[EXTRA R$20]'));
                
                if (!r.status || r.status === 'normal') {
                    // As caixas SOMAM AQUI para aparecerem grandes no cantinho esquerdo da tela (Normal e Extra)
                    if (r.tipoVeiculo === 'cacamba') totalViagensMes += (r.servicos || 0);
                    else                             totalCaixasMes  += (r.servicos || 0);
                    
                    // Aqui é a "trava da Meta". Os pontos só sobem se NÃO for extra!
                    if (!isExtra) {
        ...
        window.atualizarResumosGlobais = function () {
            ...
            for (const [dataStr, dadosDia] of Object.entries(bancoDados)) {
                ...
                for (const [mot, d] of Object.entries(dadosDia)) {
                    ...
                    const isExtra = d.observacao && d.observacao.includes('[EXTRA R$ 20]');
                    if ((!d.status || d.status === 'normal') && !isExtra) {
            ...
            for (const [dataStr, dadosDia] of Object.entries(bancoDados)) {
                if (dataEstaNoIntervalo(dataStr, inicio, fim)) {
                    ...
                    if (!isExtra) {
                        ...
                        if ((!dados.status || dados.status === 'normal') && !isExtra) {
                            const srv = dados.servicos || 0;
    ...
    // Pega a data da caixinha unificada (com fallback para garantir que não dê bug)
    ...
    try {
        ...
        (lancs || []).forEach(l => {
            ...
            // Agrupa para montar as listas de Domingos e Feriados
    ...
    _set('totalExtraDiasUteis', formatarMoeda(extraTotalDiasUteis));
    ...
    function renderizarLista(listaRegistros, idElemento, msgVazia) {
        ...
        listaRegistros.forEach(mot => {
            let textoQtd = mot.status && mot.status !== 'normal' 
                ? `<span class="bg-red-500 text-white px-2 py-0.5 rounded text-[10px] uppercase font-bold">${mot.status}</span>` 
                : formatarQuantidadeMista(mot.caixas, mot.viagens, window.motOutros.includes(mot.nome));
    ...
    renderizarLista(registrosDom, 'listaDomingos', 'Nenhum serviço em domingos.');
...
window.atualizarGraficosProjecao = async function () {
    ...
    win.print();
        ...
        window.renderizarRelatorioFaltas = function () {
            ...
            const statusLabel = { 
            falta:'Falta', atestado:'Atestado', folga:'Folga', suspensao:'Suspensão',
            polioff:'Poli OFF', licenca:'Férias', desligado:'Desligado' 
        };
        const statusColor = { 
            falta:'#dc2626', atestado:'#d97706', folga:'#64748b', suspensao:'#000000',
        ...
        window.exportarPdfFaltasAtestados = function (tipo) {
            ...
            const tiposFiltro = tipo === 'todos' ? ['falta','atestado','folga','polioff','licenca','desligado','suspensao']
            : tipo === 'falta'    ? ['falta', 'suspensao']
            : ['atestado']; 
