...
window.mudarAba = function (aba) {
   ...
   ['viewLancamentos', 'viewRankings', 'viewDomFeriados', 'viewProjecao', 'viewAuditoria', 'viewRotas', 'viewCadastro', 'viewRelCaixas', 'viewOperador'].forEach(id => {
        const el = document.getElementById(id);
    ...
    const mapaAbas = {
        ...
        relcaixas:   { btn: 'btnTabRelCaixas',   view: 'viewRelCaixas'   },
...
window.exportarRelatorioFaltas = function () {
    const elGlobal = document.getElementById('dataGlobal');
    const mesRef = elGlobal?.value ? elGlobal.value.substring(0, 7) : getAnoMesAtual();

    const bancoDados = window.bancoDadosCloud;
    const faltas = []; // [{motorista, dia}]

    for (const [dataStr, dadosDia] of Object.entries(bancoDados)) {
        if (!dataEstaNoMes(dataStr, mesRef)) continue;
        for (const mot in dadosDia) {
            const d = dadosDia[mot];
            if (d.status === 'falta') {
                const partes = dataStr.split('-');
                const diaFormatado = partes.length === 3 ? partes[2] + '/' + partes[1] + '/' + partes[0] : dataStr;
                faltas.push({ motorista: mot, dia: diaFormatado, dataRaw: dataStr });
            }
        }
    }

    if (faltas.length === 0) {
        alert('Nenhuma falta registrada no mês ' + mesRef + '.');
        return;
    }
    ...
    const porMotorista = {};
    faltas.forEach(f => {
        if (!porMotorista[f.motorista]) porMotorista[f.motorista] = [];
        porMotorista[f.motorista].push(f);
    });

    // Ordenar dias de cada motorista
    for (const mot in porMotorista) {
        porMotorista[mot].sort((a, b) => a.dataRaw.localeCompare(b.dataRaw));
    }

    // Gerar CSV
    const linhas = ['Motorista,Status,Data'];
    for (const mot of Object.keys(porMotorista).sort()) {
        for (const f of porMotorista[mot]) {
            linhas.push(mot + ',Falta,' + f.dia);
        }
    }
